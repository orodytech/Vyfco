<div class="sample bg-gradient-to-b from-blue-900 to-purple-900 flex p-4 flex-col lg:flex-row lg:px-8">
    <div class="mb-6 samples flex-wrap lg:pr-4 flex flex-col  lg:w-2/3">
        <h1 class="text-3xl font-semibold text-white">Samples</h1>
        <h3 class="text-base mb-4 text-gray-200">Have a look at samples papers done by our team</h3> 
                {{-- sample 1 --}}
                @foreach ($samples as $sample)
                <div class="item-1">
                    <div class="flex flex-col pb-4 lg:flex-row">
                        <a href="{{ url('/img/pdf/bayes1.pdf') }}" class="text-yellow-300 text-xl lg:pr-40">{{ $sample->name }}</a>
                        <p class="text-gray-400 text-sm">#{{ $sample->subject }}</p>
                    </div>
                    <div class="description-content lg:space-x-2 flex flex-col lg:flex-row">
                        <div class="description text-sm lg:text-base lg:flex-1 text-justify text-gray-200 pb-3">
                            <p>{{ $sample->description }}</p>
                        </div>
                        <div class="description flex lg:flex-1 flex-col items-center">
                            <div class="bg-black upper flex space-x-3 divide-x-2 mb-3 p-3 rounded-md shadow-md">
                                <div class="left flex-1 text-sm">
                                    <p class="text-white"><span class="text-gray-400">Number of Pages:</span> {{ $sample->number_of_pages }}</p>
                                    <p class="text-white"><span class="text-gray-400">Style:</span> {{ $sample->style }}</p>
                                    <p class="text-white"><span class="text-gray-400">Academic Level:</span> {{ $sample->level }}</p>
                                </div>
                                <div class="right pl-2 flex-1 text-sm">
                                    <p class="text-white"><span class="text-gray-400">Urgency:</span> {{ $sample->urgency }}</p>
                                    <p class="text-white"><span class="text-gray-400">Number of Sources:</span> {{ $sample->sources }}</p>
                                </div>
                            </div>
                            <div class="order-button mb-3">
                                <div class="flex items-center space-x-3">
                                    <div class="question">
                                        ...
                                        <a href="#" class="text-white text-sm">Need A paper like this?<a/>

                                    </div>
                                    <div class="button p-1 bg-gradient-to-br from-indigo-600 to-pink-700 hover:bg-gradient-to-br hover:from-blue-600 hover:to-indigo-900 text-white rounded-md text-sm">
...
<div class="side-content lg:w-1/3">
        <div class="bg-white payment shadow-lg px-4 pt-4 pb-6 mb-6 rounded-md">
            <div class="payment">
        ...
        <div class="bg-white order-form p-4 shadow-lg rounded-lg">
